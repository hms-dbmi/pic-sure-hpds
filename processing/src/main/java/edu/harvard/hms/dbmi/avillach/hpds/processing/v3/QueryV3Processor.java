...
public class QueryV3Processor implements HpdsV3Processor {
    ...
    private void writeVariantNullResultField(ResultStore results, Integer x, ByteBuffer doubleBuffer, int idInSubsetPointer) {
        byte[] valueBuffer = null;
        valueBuffer = EMPTY_STRING_BYTES;
        results.writeField(x, idInSubsetPointer, valueBuffer);
    }

    private int writeVariantResultField(
        ResultStore results, Integer x, Optional<VariableVariantMasks> variantMasks, int idPointer, int idInSubsetPointer
    ) {
    ...
    private int writeResultField(
        ResultStore results, Integer x, PhenoCube<?> cube, KeyAndValue<?>[] cubeValues, int idPointer, ByteBuffer doubleBuffer,
    ...
    /**
     * Correctly handle null records. A numerical value should be a NaN if it is missing to distinguish from a zero. A String based
     * value(categorical) should be empty instead of null because the word null might be a valid value.
     * 
     * @param results
     * @param x
     * @param cube
