...
public class AbstractProcessor {
    ...
    public AbstractProcessor(
        PhenotypeMetaStore phenotypeMetaStore, LoadingCache<String, PhenoCube<?>> store,
        Map<String, FileBackedByteIndexedInfoStore> infoStores, List<String> infoStoreColumns, GenomicProcessor genomicProcessor,
        String hpdsDataDirectory
    ) {
        this.phenotypeMetaStore = phenotypeMetaStore;
        this.store = store;
        this.genomicProcessor = genomicProcessor;
    ...
    /**
     * Merges a list of sets of patient ids by intersection. If we implemented OR semantics this would be where the change happens.
     *
    ...
    public Set<Integer> getPatientSubsetForQuery(Query query) {
        ...
        /*
         * TreeSet<Integer> idList; if(patientIdSet.isEmpty()) { if(variantService.getPatientIds().length > 0 ) { idList = new TreeSet(
         * Sets.union(phenotypeMetaStore.getPatientIds(), new TreeSet(Arrays.asList( variantService.getPatientIds()).stream()
         * .collect(Collectors.mapping( (String id)->{return Integer.parseInt(id.trim());}, Collectors.toList()))) )); }else { idList =
         * phenotypeMetaStore.getPatientIds(); } }else { idList = new TreeSet<>(applyBooleanLogic(patientIdSet)); }
         */
