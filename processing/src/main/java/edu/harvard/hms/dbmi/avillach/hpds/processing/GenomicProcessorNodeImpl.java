...
public class GenomicProcessorNodeImpl implements GenomicProcessor {
    ...
    public GenomicProcessorNodeImpl(String genomicDataDirectory) {
        ...
        if (genomicDataDirectoryFile.exists() && genomicDataDirectoryFile.isDirectory()) {
            Arrays.stream(genomicDataDirectoryFile.list((file, filename) -> {
                return filename.endsWith("infoStore.javabin");
            })).forEach((String filename) -> {
                try (
                    FileInputStream fis = new FileInputStream(this.genomicDataDirectory + filename); GZIPInputStream gis =
                        new GZIPInputStream(fis); ObjectInputStream ois = new ObjectInputStream(gis)
                ) {
    ...
    public VariantMask runGetPatientMask(DistributableQuery distributableQuery) {
        ...
        if (distributableQuery.hasFilters()) {
            ...
            if (intersectionOfInfoFilters != null) {
                patientMask = patientVariantJoinHandler
                    .getPatientIdsForIntersectionOfVariantSets(distributableQuery.getPatientIds(), intersectionOfInfoFilters);
            } else {
        ...
        for (VariableVariantMasks masks : masksForDbSnpSpec) {
            for (String zygosity : zygosities) {
                if (zygosity.equals(HOMOZYGOUS_REFERENCE)) {
                    /*
                     * // todo: implement this -- difficult with sparse variants of unknown length VariantMask homozygousReferenceBitmask =
                     * calculateIndiscriminateBitmask(masks); for(int x = 2;x<homozygousReferenceBitmask.bitLength()-2;x++) {
    ...
    private VariantIndex getComputedVariantIndexForCategoryFilter(Map.Entry<String, String[]> entry) {
        ...
        List<String> infoKeys = infoStoreOptional.map(infoStore -> filterInfoCategoryKeys(values, infoStore)).orElseGet(ArrayList::new);

        if (infoKeys.size() > 1) {
            // These should be ANDed
            return infoKeys.stream().map(key -> variantIndexCache.get(column, key)).reduce(VariantIndex::union).orElseGet(() -> {
    ...
    public Set<String> runGetVariantList(DistributableQuery query) {
        boolean queryContainsVariantInfoFilters = query.getVariantInfoFilters().stream().anyMatch(
        ...
        if (queryContainsVariantInfoFilters) {
            ...
            /*
             * if(patientSubset.size()==variantService.getPatientIds().length) { log.info("query selects all patient IDs, returning....");
            ...
            Collection<String> variantsInScope =
                variantService.filterVariantSetForPatientSet(unionOfInfoFiltersVariantSpecs, patientMaskToPatientIdSet(patientMask));

            // NC - this is the original variant filtering, which checks the patient mask from each variant against the patient mask from
            // the query
                ...
                variantsInScope.parallelStream().forEach(variantKey -> {
                    variantService.getMasks(variantKey, new VariantBucketHolder<>()).ifPresent(masks -> {
                        if (masks.heterozygousMask != null && !masks.heterozygousMask.intersection(patientMask).isEmpty()) {
                            ...
                            masks.heterozygousNoCallMask != null && !masks.heterozygousNoCallMask.intersection(patientMask).isEmpty()
                        ) {
    ...
    private VariantMask getIdSetForVariantSpecCategoryFilter(
        String[] zygosities, String key, VariantBucketHolder<VariableVariantMasks> bucketCache
    ) {
    ...
    private ArrayList<VariantMask> getBitmasksForVariantSpecCategoryFilter(
        String[] zygosities, String variantName, VariantBucketHolder<VariableVariantMasks> bucketCache
    ) {
        ...
        Arrays.stream(zygosities).forEach((zygosity) -> {
            optionalMasks.ifPresent(masks -> {
                if (zygosity.equals(HOMOZYGOUS_REFERENCE)) {
                    ...
                    /*
                     * VariantMask homozygousReferenceBitmask = calculateIndiscriminateBitmask(masks); for(int x =
                     * 2;x<homozygousReferenceBitmask.bitLength()-2;x++) { homozygousReferenceBitmask =
                     * homozygousReferenceBitmask.flipBit(x); } variantBitmasks.add(homozygousReferenceBitmask);
                     */
                } else if (masks.heterozygousMask != null && zygosity.equals(HETEROZYGOUS_VARIANT)) {
    ...
    public Set<String> getInfoStoreValues(String conceptPath) {
        return infoStores.get(conceptPath).getAllValues().keys().stream().sorted(String::compareToIgnoreCase).collect(Collectors.toSet());
    }
    ...
    public List<InfoColumnMeta> getInfoColumnMeta() {
        return getInfoStoreColumns().stream().map(infoStores::get)
            .map(
                fileBackedByteIndexedInfoStore -> new InfoColumnMeta(
                    fileBackedByteIndexedInfoStore.column_key, fileBackedByteIndexedInfoStore.description,
                    fileBackedByteIndexedInfoStore.isContinuous, fileBackedByteIndexedInfoStore.min, fileBackedByteIndexedInfoStore.max
                )
            ).collect(Collectors.toList());
    }
