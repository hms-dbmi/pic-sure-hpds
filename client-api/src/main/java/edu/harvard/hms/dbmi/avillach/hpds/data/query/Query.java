...
public class Query {
    ...
    public Query(Query query) {
        ...
        this.patientIDQuery = query.patientIDQuery;
    }
    ...
    public PatientIDQuery getPatientIDQuery() {
        return patientIDQuery;
    }

    public void setPatientIDQuery(PatientIDQuery patientIDQuery) {
        this.patientIDQuery = patientIDQuery;
    }

    private PatientIDQuery patientIDQuery = new PatientIDQuery(InclusionRule.Include, Set.of());
    ...
    public String toString() {
        ...
        if (patientIDQuery == null || patientIDQuery.rule() == null || patientIDQuery.patients() == null) {
            builder.append("Patient IDs: null\n");
        } else {
            builder.append("%s Patient IDs: [%s IDs]".formatted(patientIDQuery.rule(), patientIDQuery.patients().size()));
        }
    ...
    /**
     * For other items that are mapped (e.g., 'variable -> range') we want to show both the name and the values requested (unless
     * truncating) We can't combine this with the List/Collection method, as the two classes are not compatible (even though the method
     * names are the same)
     * @param queryPart
     * @param varMap
     * @param builder
