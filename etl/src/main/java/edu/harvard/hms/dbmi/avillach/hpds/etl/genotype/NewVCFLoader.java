...
public class NewVCFLoader {
    ...
    /**
     * @param args - if testing, this should be an array ['vcfIndexFile path', 'output storage dir', 'merged dir']. by default this will be
     *        [/opt/local/hpds/vcfIndex.tsv, "/opt/local/hpds/all", "/opt/local/hpds/merged" ].
     */
    ...
    protected TreeMap<String, FileBackedJsonIndexStorage<Integer, ConcurrentHashMap<String, VariableVariantMasks>>> variantMaskStorage =
        new TreeMap<>();
    ...
    protected void flipChunk(
        String lastContigProcessed, int lastChunkProcessed, int currentChunk, String currentContig, boolean isLastChunk, String currentLine
        ...
        if (!currentContig.contentEquals(lastContigProcessed) || isLastChunk) {
            if (infoStoreFlipped.get(lastContigProcessed) == null || !infoStoreFlipped.get(lastContigProcessed)) {
                ...
                logger.debug(
                    Thread.currentThread().getName() + " : " + "Flipping info : " + infoFile.getAbsolutePath() + " " + lastContigProcessed
                        + " "
                );
                try (
                    FileOutputStream fos = new FileOutputStream(infoFile); GZIPOutputStream gzos =
        ...
        if (!currentContig.contentEquals(lastContigProcessed) || currentChunk > lastChunkProcessed || isLastChunk) {
            // flip chunk
            TreeMap<String, FileBackedJsonIndexStorage<Integer, ConcurrentHashMap<String, VariableVariantMasks>>> variantMaskStorage_f =
                variantMaskStorage;
            ...
            chunkWriteEx.execute(() -> {
                try {
                    if (variantMaskStorage_f.get(lastContigProcessed_f) == null) {
                        ...
                        variantMaskStorage_f
                            .put(lastContigProcessed_f, new FileBackedStorageVariantMasksImpl(new File(storageDir, fileName)));
                    }
                    variantMaskStorage_f.get(lastContigProcessed_f)
                        .put(lastChunkProcessed_f, convertLoadingMapToMaskMap(zygosityMaskStrings_f));
                } catch (IOException e) {
                    throw new UncheckedIOException(e);
                }
            });
            if (lastChunkProcessed % 100 == 0) {
                logger.info(
                    System.currentTimeMillis() + " Done loading chunk : " + lastChunkProcessed + " for chromosome " + lastContigProcessed
                );
            }
            zygosityMaskStrings = new HashMap<String/* variantSpec */, char[][]/* string bitmasks */>();
        }
    ...
    protected void saveVariantStore(
        VariantStore store,
        ...
        for (FileBackedByteIndexedStorage<Integer, ConcurrentHashMap<String, VariableVariantMasks>> storage : variantMaskStorage.values()) {
    ...
    protected void saveInfoStores() throws IOException, FileNotFoundException {
        ...
        try (
            FileOutputStream fos = new FileOutputStream(new File(storageDir, "infoStores.javabin")); GZIPOutputStream gzos =
                new GZIPOutputStream(fos); ObjectOutputStream oos = new ObjectOutputStream(gzos);
        ) {
            oos.writeObject(infoStoreMap);
        }
    ...
    protected class VCFWalker implements Comparable<VCFWalker> {
        ...
        public void updateRecords(char[][] zygosityMaskStrings, ConcurrentHashMap<String, InfoStore> infoStores) {
            ...
            if (!formatIsGTOnly) {
                ...
                try {
                    ...
                    throw new RuntimeException("INDEX out of bounds " + currentLine.substring(0, Math.min(50, currentLine.length())), e);
                }
            } else {
        ...
        private void setMasksForSample(char[][] zygosityMaskStrings, int index, int startOffsetForSample) {
            int patientZygosityIndex = (currentLine.charAt(startOffsetForSample + 0) + currentLine.charAt(startOffsetForSample + 1)
                + currentLine.charAt(startOffsetForSample + 2)) % 7;
        ...
        protected String currentSpecNotation() {
            ...
            return currentLineSplit[0] + "," + currentLineSplit[1] + "," + currentLineSplit[3] + "," + currentLineSplit[4] + "," + gene
                + "," + consequence;
        }
        ...
        try {
            ...
            parser.forEach((CSVRecord r) -> {
                if (horribleHeaderSkipFlag[0]) {
                    ...
                    line.contig = r.get(CHROMOSOME_COLUMN).trim().contentEquals("ALL") ? null : r.get(CHROMOSOME_COLUMN).trim();
                    ...
                    line.sampleIds = Arrays.stream(r.get(SAMPLE_IDS_COLUMN).split(",")).map(String::trim).collect(Collectors.toList())
                        .toArray(new String[0]);
                    line.patientIds = Arrays.stream(r.get(PATIENT_IDS_COLUMN).split(",")).map(String::trim).map(Integer::parseInt)
