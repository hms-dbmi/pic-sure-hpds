...
public class VCFLoaderLiteConfig {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		// should be able to override these defaults with passed args.  Leaving them for now.
...
//			Integer[] patientIds = Arrays.stream(store.getPatientIds()).map((id)->{return Integer.parseInt(id);}).collect(Collectors.toList()).toArray(new Integer[0]);
	...
	/**
	 * In order to be dynamic data should be staged in a central location.
	 * This location is presumed to be /opt/local/hpds/
	 * By the time this loader is run the filenames should be standardized.
	 * chr needs to be included in the filename and must end with vcf.gz.
	 * 
	 * reference genome should be added as a requirement when support for non-hg38 is introduced.
	 * 
	 * while filename is not ideal currently the vcf headers do not contain the reference genome besides maybe 
	 * in the bcftools command history ( if it is provided ).
	 * 
	 * ex: EOCOPD_phs000946_TOPMed_WGS_freeze.9b.chr22.hg38.vcf.gz
	 * 
	 * Changes should interrupt downstream processes in the VCFLoader for now.
	 * @param vcfIndexFile
	 * @return
	 */
	private static List<VCFIndexLine> parseVCFIndex(File vcfIndexFile){
		...
		// it is presumed all files will be in same directory as the vcfIndex
		File vcfDirectory = vcfIndexFile.getParentFile();
		
		if(vcfDirectory.isDirectory()) {
			
			// masterrecord stores static info for each line
			VCFIndexLine vcfIndexMasterRecord = getVCFIndex(vcfIndexFile);
			
			File[] vcfFiles = vcfDirectory.listFiles(new FilenameFilter() {
			...
			for(File currentVCFFile : vcfFiles) {
				
				VCFIndexLine vcfIndexNewLine = new VCFIndexLine();
				
				vcfIndexNewLine.vcfPath = currentVCFFile.getAbsolutePath();
				vcfIndexNewLine.contig = findContig(currentVCFFile);
				vcfIndexNewLine.isAnnotated = vcfIndexMasterRecord.isAnnotated;
				vcfIndexNewLine.isGzipped = vcfIndexMasterRecord.isGzipped;
				vcfIndexNewLine.sampleIds = vcfIndexMasterRecord.sampleIds;
				vcfIndexNewLine.patientIds = vcfIndexMasterRecord.patientIds;
				
				if(!vcfIndexNewLine.contig.isBlank()) {
					vcfSet.add(vcfIndexNewLine);
				}
				
			}
		} else {
			logger.error("VCFIndex is not located in /opt/local/hpds/vcfIndex.tsv");
			throw new RuntimeException("VCFIndex is not located in /opt/local/hpds/vcfIndex.tsv", new RuntimeException());
		}
		return new ArrayList<>(vcfSet);
	}
	/**
	* Parses file name looking for chromosome / contig name.
	*/
	private static String findContig(File currentVCFFile) {
		for(String fileNamePart: currentVCFFile.getName().split("\\.")) {
			if(fileNamePart.startsWith("chr")) return fileNamePart;
		} 
		// if returning an empty string report an error stating the filename does not contain a chr value
		logger.error(currentVCFFile.getName() + " does not contain a chr value - unable to parse.");
		return "";
	}

	/**
	 * For dynamic loading it is presumed that all vcf files will be formatted in a similar manner
	 * All of these prereqs can be performed by bcftools
	 * - gzipped
	 * - no annotations
	 * - chr removed from CHROM col
	 * @param vcfIndexFile
	 * @return
	 */
	private static VCFIndexLine getVCFIndex(File vcfIndexFile) {
		CSVParser parser;
		...
		try {
			...
			// removing this can handle headers better than this.
			// final boolean[] horribleHeaderSkipFlag = {false};
			
			// get the sample ids and patient nums from the vcfIndex file
			for(CSVRecord r: parser.getRecords()) {
				if(!r.get(0).trim().equalsIgnoreCase("vcf_path")) {
					...
					line.isAnnotated = false;//Integer.parseInt(r.get(ANNOTATED_FLAG_COLUMN).trim())==1;
					line.isGzipped = true;//Integer.parseInt(r.get(GZIP_FLAG_COLUMN).trim())==1;
			...
			/*
			 * for this dynamic parsing only information stored in the vcf index file is the sampleIds and patientNum mappings
			 
			parser.forEach((CSVRecord r)->{
				
				if(!r.get(0).trim().equalsIgnoreCase("filename")) {
