...
public class QueryService {
    ...
    private boolean ensureAllFieldsExist(Query query) {
        ...
        Set<String> catFieldNames =
            query.getCategoryFilters().keySet().stream().filter(Predicate.not(VariantUtils::pathIsVariantSpec)).collect(Collectors.toSet());
        ...
        List<String> missingFields = allFields.stream().filter(Predicate.not(dictionaryFields::contains)).collect(Collectors.toList());
